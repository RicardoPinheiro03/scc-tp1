...
public class AzureManagement
{
    ...
    public static StorageAccount createStorageAccount(Azure azure, String rgName, String name, Region region) {
        ...
        System.out.println( "Storage account created with success: name = " + name + " ; group = " + rgName + " ; region = " +  region.name());
    ...
    private static BlobContainer createBlobContainer(Azure azure, String rgName, String accountName,
        ...
        System.out.println( "Blob container created with success: name = " + containerName + " ; group = " + rgName + " ; account = " +  accountName);
        return container;
    }

    public static void dumpStorageKey(String propFilename, StorageAccount account) throws IOException {
        ...
        Files.write(Paths.get(propFilename), ("BLOB_KEY=DefaultEndpointsProtocol=https;AccountName=" +
                        account.name() + ";AccountKey=" + storageAccountKeys.get(0).value() + ";EndpointSuffix=core.windows.net\n").getBytes(),
        ...
        synchronized(AzureManagement.class) {
            Files.write(Paths.get("./src/scc/configs/set-app-config.sh"), cmd.toString().getBytes(),
    ...
    public static CosmosDBAccount createCosmosDBAccount(Azure azure, String rgName, String name, Region region, Region region2) {
        WithConsistencyPolicy step = azure.cosmosDBAccounts().define( name)
                .withRegion(region)
                .withExistingResourceGroup(rgName)
                .withDataModelSql();
        CosmosDBAccount account = null;
        if( region2 != null) {
            account = step.withSessionConsistency()
                    .withWriteReplication(region2)
            ...
            account = step.withSessionConsistency()
                    ...
                    .create();
        System.out.println( "CosmosDB account created with success: name = " + name + " ; group = " + rgName + " ; region = " +  region.name() + " ; region 2 = " + (region2 == null ? "null" : region2.name()));
        return account;
    }

    public static void dumpCosmosDBKey(String propFilename, CosmosDBAccount account) throws IOException {
        account.regenerateKey(KeyKind.PRIMARY);
        Files.write(Paths.get(propFilename), ("COSMOSDB_KEY=" + account.listKeys().primaryMasterKey() + "\n").getBytes(),
                StandardOpenOption.APPEND);
    ...
    static void createDatabase(AsyncDocumentClient client, String dbname) {
        ...
        List<Database> databaseList = client
                .queryDatabases("SELECT * FROM root r WHERE r.id='" + dbname + "'", null).toBlocking()
                .first().getResults();
    ...
    static void createCollection(AsyncDocumentClient client, String dbname, String collectionName, String partKeys, String uniqueKeys) {
        ...
        try {
            if (args.length == 1 && args[0].equalsIgnoreCase("-delete")) {
                ...
                Files.write(Paths.get(AZURE_PROPS_LOCATION),
                        ...
                        StandardOpenOption.CREATE, StandardOpenOption.WRITE);
                Files.deleteIfExists(Paths.get(AZURE_PROPS_LOCATION));
                Files.write(Paths.get("./src/scc/configs/set-app-config.sh"), "".getBytes(),
                        StandardOpenOption.CREATE, StandardOpenOption.WRITE);
